...
class GeneticAlgorithm:
    def __init__(self, city_num, city_dist_mat: np.array, pop_size=50, generations=20,
    ...
    def _crossover(self) -> list[Individual]:
        ...
        population.sort(key=lambda ind: ind.evaluate_fitness(), reverse=True)
    ...
    def _mutation(self, population: list[Individual]) -> list[Individual]:
        ...
        population_copy.sort(key=lambda ind: ind.evaluate_fitness(), reverse=True)
    ...
    def plot_best_route(self):
        """绘制最终的最优路线，使城市近似正多边形分布"""
        plt.figure(figsize=(8, 6))
        plt.title(f"Best Route Found (Distance: {self.best.calculate_distance()})")

        # 计算正多边形坐标（城市围绕中心点均匀分布）
        city_num = self.city_num
        radius = 50  # 设定一个固定半径，控制城市分布范围
        center_x, center_y = 100, 100  # 圆心
        angles = np.linspace(0, 2 * np.pi, city_num, endpoint=False)  # 角度均分
        ...
        # 获取最优路径
        best_route = self.best.genes + [self.best.genes[0]]  # 回到起点

        # 提取坐标点
        x_data = [city_positions[i, 0] for i in best_route]
        y_data = [city_positions[i, 1] for i in best_route]

        # 绘制城市点
        plt.scatter(city_positions[:, 0], city_positions[:, 1], c='red', s=100, label="Cities")

        # 绘制最优路线
        plt.plot(x_data, y_data, 'r-o', label="Best Path")

        # 标记城市编号
        for i, (x, y) in enumerate(city_positions):
            plt.text(x + 2, y + 2, str(i), fontsize=12, color='black')
